...
class CustomerMemoryStore:
    ...
    def __init__(self, settings:Settings, llm:Any):
        ...
        config: dict[str, Any] = {
            "llm": {
                "provider": "groq",
                "config": {
                    "model": settings.memory_groq_model,
        ...
        if settings.google_api_key:
            ...
            raise RuntimeError(
                ...
                "or OPENAI_API_KEY. Set ENABLE_LOCAL_EMBEDDINGS=true to use local Ollama embeddings."
            )
    ...
    def search(self, query: str, user_id: str, limit: int = 5) -> list[dict[str, Any]]:
        try:
            raw = self._memory.search(query, filters={"user_id": user_id}, top_k=limit)
        except TypeError:
    ...
    def add_resolution(
        ...
        memory_text = (
            "Accepted support resolution.\n"
            f"Ticket: {self._truncate(ticket_subject, 160)}\n"
            f"Issue: {self._truncate(ticket_description, 300)}\n"
            f"Resolution: {self._truncate(accepted_draft, 500)}{entity_text}"
        )
        messages = [
            {
                "role": "user",
        ...
        self._add_messages(messages=messages, user_id=user_id, metadata=metadata, infer=False)
    ...
    def _add_messages(
        ...
        try:
            self._memory.add(messages, user_id=user_id, metadata=metadata or {}, infer=infer)
        except TypeError:
